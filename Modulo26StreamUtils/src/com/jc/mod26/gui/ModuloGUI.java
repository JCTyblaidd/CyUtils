...
public class ModuloGUI {
	...
	private void stripCipherText() {
		splits.clear();
		splits.add(cipher);	//HAX NOW MEANS THAT THERE ISN'T THE LUXARY OF NO KEYSTREAM
		/**
		int curr = 0;
		String temp = "";
	...
	public void OnFirstChanged() {
		...
		try{
			...
			relatedFirst = "???????????????????????";
									...
									//SUBSTRACT THE KEYSTREAM FROM THE CIPHER - PLAIN
		String key = Modulo26Utils.handleStreams(relatedFirst,saltedFirst,false);
		//KEY
	...
	public void OnKeyChanged() {
		...
		String relatedFirst = this.splits.get(0);
										//EXTRACT FIRSTREAM FROM CIPHER - KEY
		String first = Modulo26Utils.handleStreams(relatedFirst,saltedKey,false);
	...
	protected void genOutputFromKEY(String key) {
		...
		String output = "";
		for(String str : splits) {			//GENERATE PLAINTEXT FROM CIPHER + KEY
			String undo = Modulo26Utils.handleStreams(str, key,true);
			output = output + undo;
		}
	...
	protected class ReflectionHandler implements InputMethodListener,KeyListener{
		...
		public ModuloGUI link;
		public String linked_function;
		private Method function;
		
		public ReflectionHandler(ModuloGUI link,String funct) {
			this.link = link;
