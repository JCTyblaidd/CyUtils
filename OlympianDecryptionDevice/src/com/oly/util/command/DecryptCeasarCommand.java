...
public class DecryptCeasarCommand implements ITextWindowCommand{
	...
	public void run(String[] args, TextWindow link) {
		if(args.length != (2 + 1)) {
			//System.err.println("DEBUG:" + args.length);
			link.write_col("ceasar [text/{WEBID}] [shift]", Color.RED);
			return;
		}else {
			String toDecrypt = getText(args[0+1].trim());
			int i = Integer.parseInt(args[1+1].trim());
			link.write_col(KeyDecrypter.shift(toDecrypt,(26- i)%26),Color.GRAY);
		}
	}
